...
void Mesh::createBuffer(Vertex vertices[], int vn, unsigned int  indices[], int in, ID3D11Device * device) {
	inte = in;
	//cout << vertices[0].Color.x;
	D3D11_BUFFER_DESC vbd;
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex) * vn;       // 3 = number of vertices in the buffer
	...
	ibd.ByteWidth = sizeof(int) * in;         // 3 = number of indices in the buffer
...
Mesh::Mesh(Vertex  vertices[], int vn, unsigned int  indices[], int in, ID3D11Device * device)
{
	createBuffer(vertices, vn, indices, in, device);
}
